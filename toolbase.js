...
function getJsonWithEmptyBody(url, headers = null) {
    if (typeof _apiFetcher === 'undefined') {
        return {};
    }
...
tools.add({
    id: "getJsonWithEmptyBody",
    impl: getJsonWithEmptyBody,
    aliases: {
        en: "getJsonWithEmptyBody",
        de: "getJsonWithEmptyBody"
    },
    simpleDescription: {
        de: "JSON-Daten von API mit leerem Body abrufen",
        en: "Fetch JSON data from an API with empty body"
    },
    argsOld: {
        en: "URL, header",
        de: "URL, header"
    ...
    tests: () => {
        tools.it('will return empty object if no apiFetcher is defined', () => {
            tools.expect(getJsonWithEmptyBody("https://some.interesting.url")).jsonToBe({});
        });

        tools.it('will call the endpoint with empty body', () => {
            try {
                _apiFetcher = {
                    postUrl: (url, body, header) => {
                        tools.expect(body).toBe('{}');
                        return JSON.stringify({});
                    }
                }

                getJsonWithEmptyBody("https://some.interesting.url");
            } finally {
                delete _apiFetcher;
            }
        });
        ...
        tools.it('will use application/json as default content-type', () => {
            try {
                _apiFetcher = {
                    postUrl: (url, body, header) => {
                        tools.expect(header['content-type']).toBe('application/json');
                        return JSON.stringify({});
                    }
                }
